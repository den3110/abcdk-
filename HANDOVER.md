> 🆕 **Session 2026-09-13 (Native Live: fix chuyển trận + audit 10h + Observer tự chủ + Icon + LIVE ĐA ĐÍCH):** HEAD `abcdk-` master **`59563738`** (đã push + **deploy VPS**). Trọng tâm: (A) **fix chuyển trận + tự GO LIVE** 2 app — overlay tên vòng ("Vòng 1 - Pre-Qualifying" thay "Chung kết", ưu tiên `bracket.name`); FB busy theo TỪNG page (không cả owner); backend publish `court-station:update` khi trận **start** + `patchWinner` dọn currentMatch sân manual; iOS `armedStartWatchdog` 2s giữ ý định live xuyên trận kết thúc (parity Android autoGoLive). (B) **audit chạy 10h** (2 agent) — iOS: upload loop nóng CPU, **recorder nhiễm độc vĩnh viễn** (HaishinKit finishWriting guard-fail → `recorder` thành `var`+rebuild), tự hồi RTMP `.failed`, dừng ghi khi đầy đĩa; Android: xoá segment mồ côi, không tái tạo GL filter mỗi rotate, backoff reconnect 3s→60s. (C) **Android Foreground Service** (`camera|microphone` + wake lock 14h — không có thì Android 11+ thu hồi camera khi màn tắt/cuộc gọi/Home → rớt live) + **cameraDispatcher** (stopRecord/startRecord khỏi Main, hết ANR). (D) **Observer TỰ CHỦ** — port live_device.go → Node (model `LiveDeviceState`, `/api/observer/ingest/live-devices/*` qua protectLiveApp), dashboard **`https://observer.pickletour.vn/observer/live-devices`** (nginx→5001+certbot, read-key `pt_obs_read_...`), trang admin `/admin/observer-vps` đọc Mongo local, `SystemSettings.links.liveObserverUrl` đã trỏ. (E) **Icon app mới** (2 app, nguồn logo tròn → ImageMagick). (F) ⭐ **LIVE ĐA ĐÍCH** (client-side fan-out) — nhiều FB page + YouTube CÙNG LÚC từ 1 điện thoại: backend `POST /live-app/matches/:id/live/create-multi` tạo N live; iOS `SecondaryRTMPOutput` (RTMPStream passthrough qua IOStreamObserver, 1 composite N encode); Android Pedro `MultiCamera2` (migrate RtmpCamera2, slot 0=chính giữ nguyên logic); UI "Phát nhiều đích". ⚠️ **CHƯA test máy thật** (chỉ compile) — user build TestFlight iOS + cài **APK v21** (`~/Desktop/pickletour-live-android/pickletour-live-v1.2026.09.13.021.apk`) rồi test. Băng thông đa đích × N — giữ 2–3 đích. Đọc `HANDOFF.md §đầu` + memories `[[observer-live-devices]]` `[[native-live-multidestination]]` `[[native-live-fanpage-select]]`.
>
> 🆕 **Session 2026-09-12→13 (Native Live iOS parity Android + FB live):** 16 commit đưa `native-live-app-ios` (repo `abcdk-`) từ "không live được / crash / không overlay / video FB không khả dụng" → **live được cả FB/YouTube, overlay pixel-parity Android, tỉ số realtime, ghi hình OK**. Trọng tâm: (1) **RTMP FB** — HaishinKit tách connect+publishName phải giữ query `?...&a=<auth>` trong publishName; (2) **Crash bg-thread `@Published`** — `@objc` HaishinKit callback phải `nonisolated`+`Task { @MainActor }`; (3) **Overlay không render** — HaishinKit mặc định `.passthrough` (VideoEffect KHÔNG chạy); phải bật `.offscreen` + `screen.startRunning()` (pod KHÔNG tự start); encoder `videoSettings.videoSize` mặc định 854x480; (4) **Overlay pixel-parity Android** — port `OverlayBitmapRenderer.kt` (basis 1280x720, uiScale, card 520x160 blit 0.75, text baseline, OverlayNameStyle, sponsor bar 8 logo); (5) **Tỉ số realtime** — DTO socket KHÔNG có scoreA/scoreB, suy từ `gameScores[currentGame]` như Android; `match:update` bọc `{data}`; patch-only → `match:snapshot:request`; (6) **FB "video không khả dụng"** — link `watch/live/?v=<liveId>` là format cũ (dùng permalink FB trả về); tái dùng live cũ chỉ dựa DB (phải hỏi Graph); ép `fbGoLive` sau 8s như admin live-test; (7) **Recorder** `IOStreamRecorder.Error 3` — settings audio phải để 0 (mixer 48kHz vs ép 44.1); (8) **Finalize 409** — `abandonFailedSegments`; (9) **Preview** aspect (fill crop mất overlay); (10) **ATS** upgrade http→https ảnh; (11) Backend `foundMdatAtom is not defined` (ReferenceError). ⚠️ **CHƯA DEPLOY BACKEND** — user chạy: `SSHPASS='Hoang@07082026' sshpass -e ssh -o StrictHostKeyChecking=no root@103.90.225.130 "cd /abcdk- && git fetch origin master && git reset --hard origin/master && pm2 restart server --update-env"`. Đọc `HANDOFF.md` §đầu + memories `[[native-live-ios-*]]` `[[live-fb-video-unavailable]]`. iOS chưa build TestFlight — user tự build từ Xcode.

> 🆕 **Session 2026-09-11→12:** **Giám sát vận hành + Telegram** (token hết hạn/SSL/5xx/tồn đọng — Admin→Cài đặt→Giám sát vận hành, `SystemSettings.opsMonitor`, `backend/services/ops/*`) · **Admin FB Login Tokens** (`/admin/fb-boot-tokens` — token=tài khoản) · **FB/YouTube Live Test** (`/admin/fb-live-test`, `/admin/yt-live-test`; 🔴 **ffmpeg-static SEGFAULT khi RTMPS → dùng /usr/bin/ffmpeg `FFMPEG_PATH`**; YT quota 10k/ngày) · **Chat link bấm được + copy** (web+mobile) · **POS khách vãng lai** (gộp tiền sân/dịch vụ + bán hàng 1 bill, `VenueSale.serviceItems`; sửa/xoá đơn) · **Điểm "Chưa chấm"** (cờ `rated`; 2.5 là default localRatings) · **SĐT/CCCD bàn phím hệ thống** (nhập số 0 đầu; ⚠️ OTP chưa fix) · **MLP**: giải demo `6aa4b00aa456bb8de2d7d320` + web v3 hiện đội MLP + fix màu dark (`MlpBracketView`) · iOS 1.1.15/45 + fix launch screen + native-live iOS · **Thẻ giải MLP đếm đội** (list/detail/admin lookup `mlpteams` thay vì chỉ `registrations`; FE ẩn mẫu số khi `maxPairs=0`). ⚠️ **Mobile ở nhánh `feat/ui-v2-modern`**. Đọc `../HANDOFF.md` + `HANDOFF.md §đầu`. Đã deploy hết (backend+web+admin+OTA).

> 🆕 **Session 2026-09-07:** Đặt sân (Alobo-style) hoàn thiện + **Nhân viên & phân quyền** (12 quyền, vai trò Quản lý/Thu ngân/Nhân viên; BE `venueStaffModel`/`venueAuth.venueCan`; mobile `owner/venue/[id]/staff.tsx` + web `VenueStaffPage.jsx`) + **POS in bill/PDF** (expo-print, `receiptHtml.ts`) + **Bank picker có logo** (52 ngân hàng từ PickleBook; mobile `BankPicker.tsx`, web `BankSelect.jsx`) + **giữ chỗ 15'** + fix **"Ngày không hợp lệ"** (middleware `normalizeRequestDates` ép body.date→Date, đã skip prefix booking). 🔴 **QUY TẮC BÀN PHÍM CHE INPUT — xem HANDOFF §đầu, MỌI màn/modal có TextInput phải dùng KeyboardAvoidingView / `automaticallyAdjustKeyboardInsets`.** Backend cần deploy: `cd /abcdk- && git fetch origin master && git reset --hard origin/master && pm2 restart server --update-env`.

> 🆕 **Session 2026-08-27→28:** Zalo ZNS OTP toàn diện (đăng ký/kích hoạt SĐT/reset mật khẩu/gate hành động, rate-limit 3/ngày+60s, trang Logs admin, cấu hình access_token ở admin) + CLB chat nhóm/quỹ/buổi tập/BXH + **parity + thiết kế lại + fix light-mode giao diện V2 (Astryx)** + tin nhắn popup ở V2 + toggle giới hạn tìm kiếm + fix bộ lọc vượt trình. **ĐÃ DEPLOY HẾT** (backend+web+admin+OTA 4/4). **Đọc `HANDOFF.md` §đầu.** ⚠️ Deploy phiên này qua **SSH thẳng VPS `root@103.90.225.130`** (không dùng CI/clone-tmp) — xem §"CÁCH DEPLOY". 🔴 **Video Drive lỗi cần user tự xử lý** (mất khoá `LIVE_SECRET_KEY_BASE64` → kết nối lại Google Drive ở admin).
