...
> - **⭐ Quét mã QR CCCD chip + Admin Kiểm tra CCCD có nút "Đọc CCCD (OCR)":** thêm `backend/services/ocr/cccdQrDecoder.js` — jsQR + canvas, thử 4 orientation × 3 scale × 4 quadrant (bắt QR ảnh lớn/nhỏ/xoay). Parse payload chuẩn Bộ CA: `<CCCD>|<CMND cũ>|<Họ tên>|<ddmmyyyy DOB>|<Nam/Nữ>|<địa chỉ thường trú>|<ddmmyyyy ngày cấp>`. Quét trước OCR — nếu hit thì merge (ghi đè) idNumber/fullName/dob/sex/residence/issueDate vào kết quả (chính xác 100%). Chấp nhận QR ở CẢ mặt trước và mặt sau (loop qua tất cả ảnh truyền vào). `_usage.qr=true` khi hit. Admin `UserListPage`: dialog "Kiểm tra CCCD" thêm panel "Đọc thông tin bằng OCR" (chip Cedrus OCR) + IconButton scanner ở mỗi row → tự chạy OCR khi mở dialog; bảng so sánh 5 field DB vs OCR, nút "Chỉ điền trường trống" / "Ghi đè theo OCR" gọi `POST /admin/users/:id/ai-cccd` (dispatcher Cedrus → QR → OCR → Claude fallback).
...
> ```bash
> SSHPASS='Hoang@07082026' sshpass -e ssh -o StrictHostKeyChecking=no root@103.90.225.130 "cd /abcdk- && git fetch origin master && git reset --hard origin/master && pm2 restart server --update-env"
> ```
> để đưa lên các fix backend + web (link FB permalink thật, siết reuse live, ép LIVE_NOW, fix ReferenceError `foundMdatAtom`).
>
> **Trọng tâm phiên:** người dùng test iOS live thật lên FB, gặp hàng loạt lỗi. Chuỗi fix (16 commit) đưa native-live-app-ios từ "không live được / crash / không overlay" → **live được cả FB/YouTube, overlay pixel-parity Android, tỉ số realtime, ghi hình không lỗi**.
>
> ### 🎯 Chuỗi fix iOS (16 commit, đọc theo thứ tự để hiểu):
> 1. **`5ab62214` FB RTMP auth token** — `RTMPDestination.parse` tách sai URL FB. `secure_stream_url` dạng `rtmps://…/rtmp/<id>?s_bl=…&a=<auth>` — token `a=` phải đi CÙNG stream key khi publish. HaishinKit tách connect+publishName ⇒ query phải theo publishName, KHÔNG nhét vào connectURL. Android/Pedro nhận nguyên URL nên không dính. `[[native-live-ios-rtmp-fb-authtoken]]`.
> 2. **`a07773cf` Crash/decode/ATS** — (a) callback HaishinKit `handleRTMPStatus/Error` (`@objc`, thread nền) mutate `@Published` của class `@MainActor` → "Publishing changes from background threads" → **treo+crash**. Đánh dấu `nonisolated` + hop `Task { @MainActor in }`. (b) DTO có `sets/occupied` dạng object, model iOS đợi `[SetScore]/Bool` → JSONDecoder ném → mất snapshot. Thêm `@propertyWrapper Lenient` (field lệch kiểu → nil). (c) ATS chặn ảnh `http://` (log "does not conform to ATS policy") → `String.httpsUpgraded` trong `loadRemoteImage` + `AsyncImage`. `[[native-live-ios-crash-decode-ats]]`.
> 3. **`159f2321` ROOT CAUSE overlay không render** — HaishinKit 1.9.9 mặc định `IOVideoMixerSettings.mode = .passthrough` → IOVideoMixer.append đẩy thẳng frame ra output, VideoEffect **KHÔNG BAO GIỜ chạy**. Effect chỉ execute trong `.offscreen` (Screen → VideoTrackScreenObject.makeImage). Pod cũng **KHÔNG tự start** `DisplayLinkChoreographer`. FIX: `videoMixerSettings.mode = .offscreen` + `stream.screen.startRunning()` (idempotent) + `stopRunning()` ở stopPreview/deinit + `screen.size` khớp hướng xoay + `videoSettings.videoSize` (mặc định HaishinKit 854x480!) đồng bộ theo preset — trước đây stream "1080p" thực tế 854x480. `[[native-live-ios-haishinkit-offscreen]]`.
> 4. **`1a79371c` Overlay pixel-parity Android** — port nguyên văn `OverlayBitmapRenderer.kt` từ spec 5 agent rút ra. Basis 1280x720, uiScale=min(w/1280,h/720), margin 16; card 520x160 blit 0.75 → basis (16,16,390,120); text theo BASELINE (origin.y = baseline − font.ascender). 3 layer: card (normal/break/MLP) → logo box 68 góc phải trên (web ưu tiên giải) → sponsor bar góc phải dưới ô 44.8 pad 5.6 tối đa 8, kéo giãn vuông. Port OverlayNameStyle (style 1-4, thang cỡ chữ, bóp ngang khi tràn). Model thêm `isBreak/overlayNameStyle` vào `LiveOverlaySnapshot/MatchData/TournamentInfo`; enrich không chèn "PickleTour"/"Court" nữa. Xoá badge debug "WEB… | SPONSOR…".
> 5. **`a8ce6e4e` Tỉ số realtime** — DTO socket trận giải (toRealtimePublicMatchDTO) **KHÔNG có scoreA/scoreB** — chỉ `gameScores[{a,b}]` + `currentGame` (0-based) + `serve{side,server}`. iOS decode thành snapshot toàn nil, enrich không điền → overlay luôn 0-0. `match:update` bọc `{type, matchId, data: DTO}`. FIX: `LiveOverlaySnapshot` thêm `gameScores/currentGame/serve` + `withDerivedLiveState()` (port Android extractCurrentScore: `gameScores[currentGame]` → `.last`). `LiveSockets.handleMatchPayload` bóc envelope `data`; patch-only → emit `match:snapshot:request`. SetScore decode mềm (DTO không có `index`).
> 6. **`4c6a6955` Socket bền như Android** — guard matchId (String/NSNumber/`{$oid}`); gate `liveVersion` (bỏ payload cũ hơn bản đã áp, strict <); `merging(over:)` — field trong payload mới thắng, thiếu thì giữ snapshot trước (activeMatch không bao giờ ghi đè dữ liệu socket); onStationUpdate chỉ cập nhật status (DTO station không có điểm/seed/logo, nay decode được nên ghi đè sẽ mất seed); health khi payload stale mà đã join đúng room → xin snapshot; `MatchData.isBreak` decode cả boolean lẫn object `{active,note,type}` (runtime trả object).
> 7. **`36ea23c7` FB "Video trực tiếp không khả dụng"** — 3 khác biệt vs `/admin/fb-live-test` (không bao giờ dính): (a) link viewer dạng cũ `facebook.com/watch/live/?v=<liveId>` — FB không còn resolve. `buildFacebookUrls` giờ ưu tiên `raw_permalink_url` (permalink LIVE thật) → video permalink → dạng cũ chỉ fallback; đồng bộ cả `publicStreams`, web `LiveMatchCard`/`MatchContent`. (b) tái dùng live cũ chỉ dựa DB `status "LIVE"` (phiên chết không gọi `/live/end` để lại "LIVE" tới ~5 phút = lease 120s + grace 180s) → app đẩy vào key của live FB đã kết thúc. Nay chỉ tái dùng <60s VÀ `fbGetLiveVideo` xác nhận status còn nhận; từ chối → đánh dấu `STALE`. (c) app-path chưa bao giờ ép `LIVE_NOW`; admin gọi `fbGoLive` sau 8s. `notifyStreamStarted` giờ lên lịch `fbGoLive` sau 8s cho facebook. Kèm: cron lease chỉ end đúng `lease.liveVideoId` (không end nhầm live mới). `[[live-fb-video-unavailable]]`.
> 8. **`6831911d` iOS force=true** — `LiveAppStore.goLive()` truyền `force: true` khi tạo live FB (luôn tạo mới như admin, không tái dùng).
> 9. **`a28eaeda` Preview đủ khung + recorder audio + log** — (a) preview MTHKView `.resizeAspectFill` → `.resizeAspect`: iPhone 19.5:9 fill frame 16:9 crop mất thanh tên giải lề trên; nay WYSIWYG như Android (viền đen 2 bên). (b) `IOStreamRecorder.Error error 3` = `failedToFinishWriting`: app ép recorder audio AAC **44.1kHz**/1ch trong khi mixer chạy **48kHz** → input audio không tạo/append được → writer không vào `.writing` → mỗi 6s xoay segment báo lỗi. Nay `recorder.settings` = 0 (HaishinKit lấy theo NGUỒN thật — đúng mặc định thư viện); segment rỗng → mở segment mới ≤2 lần thay vì hạ ghi hình. (c) `@Lenient` cho scoreA/scoreB/serveCount/seedA/seedB/currentGame — một field lệch kiểu không nuốt cả payload. (d) log `[PTLive socket]/[PTLive court]/[PTLive match]` để chẩn đoán.
> 10. **`607f5a1c` Finalize 409** — vòng upload chạy ngay lúc app khởi động; phiên trước ghi hình lỗi để lại segment server "pending" mà client không còn file nào tải → backend trả 409 vô hạn → banner đỏ mỗi lần mở app. FIX: `FinalizeMatchRecordingRequest` thêm `abandonFailedSegments`; `finalizeRecordingIfPossible`: 409 → thử lại với `abandonFailedSegments=true` (backend đã support); 400/404 → gỡ pending finalize không retry.
> 11. **`5269fed5` Backend `foundMdatAtom is not defined`** — trong `liveRecordingV2Controller.js` khai báo `let foundMoovAtom/foundMdatAtom` trong `try` nhưng đọc `if (!foundMdatAtom)` sau `try/catch` → ReferenceError khi segment scan chưa đủ atom → app hiện chuỗi lỗi này khi dừng live. Hoist 2 biến ra ngoài `try`.
>
> ### 🧠 Kiến trúc iOS live (đọc trước khi sửa gì thêm)
> - **`native-live-app-ios/PickleTourLive`** — SwiftUI, `LiveAppStore` (@MainActor, ObservableObject), HaishinKit 1.9.9 (Pods/, gitignored), Socket.IO-Client-Swift, AppAuth, Firebase (chưa `configure()` — Bundle ID lệch `GoogleService-Info.plist`, Crashlytics không nhận).
> - **Bundle ID canonical**: `com.pkt.pickletour.live`. Team `AYGUWW47SB` (huyenhin20@gmail.com).
> - **Đường dữ liệu overlay**: socket `match:snapshot|score:updated|match:update|match:patched|status:updated|winner:updated|match:joined` → `MatchSocketCoordinator.handleMatchPayload` (bóc envelope, guard matchId, gate version) → `LiveOverlaySnapshot.withDerivedLiveState()` (suy điểm/serve từ gameScores) → `LiveAppStore.onOverlaySnapshot` → `merging(over:)` → `enrichOverlaySnapshot` (điền chỗ trống từ activeMatch) → `streamingService.overlaySnapshot` → `LiveScoreboardOverlayRenderer.overlayImage(for:)` → `CISourceOverCompositing` → HaishinKit offscreen pipeline → RTMP + recording.
> - **QUY TẮC HaishinKit 1.9.x** (`[[native-live-ios-haishinkit-offscreen]]`): (1) `videoMixerSettings.mode = .offscreen` (mặc định passthrough → effect không chạy). (2) Pod **KHÔNG** tự `screen.startRunning()` — phải gọi thủ công. (3) `screen.size` khớp hướng xoay + aspect camera (`.resizeAspect`, top-align → sai aspect ⇒ viền đen dưới). (4) Encoder H264 theo `videoSettings.videoSize` (mặc định 854x480!), KHÔNG phải `screen.size`. (5) Effect output phải `.cropped(to: image.extent)` — CISourceOverCompositing trả union extent; renderer CPU pod không clip → tràn buffer. (6) Fail-soft `.disabled` phải về passthrough + stop loop, không chỉ null overlay. (7) Recorder `settings` để 0 (audio/video 0kHz/0px) → tự lấy theo NGUỒN thật; ép cứng sample rate gây `failedToFinishWriting`.
> - **QUY TẮC HaishinKit callback**: mọi `@objc` listener (`handleRTMPStatus/Error`) phải `nonisolated` + hop `Task { @MainActor in }` trước khi chạm state; callback closure (recorder proxy) đã hop qua `Task { await }` nên OK.
> - **DTO trận giải KHÔNG có scoreA/scoreB** — suy từ `gameScores[currentGame]` như Android extractCurrentScore. `match:update` bọc `{data}`. Patch-only (`match:patched`, `status:updated`) → xin `match:snapshot:request`.
> - **Link FB viewer** phải là **permalink FB trả về** (`raw_permalink_url`), không phải `watch/live/?v=<liveId>` (FB không resolve — "Video trực tiếp không khả dụng").
> - **Overlay parity Android** — spec rút từ `native-live-app/app/src/main/java/com/pkt/live/streaming/OverlayBitmapRenderer.kt`, ghi trong `native-live-app-ios/PickleTourLive/LiveStreamingService.swift` (methods `drawNormalCard/drawBreakCard/drawMlpCard/drawLogoBox/drawSponsorBar` + helpers `truncateText/chooseFittedTeamName/parseTeamColor`).
>
> ### 🚧 Còn tồn (chưa fix, không phải lỗi chức năng)
> - **Firebase chưa `FirebaseApp.configure()`** + Bundle ID lệch `GoogleService-Info.plist` → Crashlytics không báo cáo. Cần: (a) thêm `FirebaseApp.configure()` vào `PickleTourLiveApp.init()`; (b) download lại `GoogleService-Info.plist` từ Firebase console cho bundle `com.pkt.pickletour.live` (hiện file trong repo là bundle khác).
> - **Font parity** — dùng SF của iOS, Android dùng Roboto → truncate name có thể lệch ~1 ký tự. Muốn giống tuyệt đối: bundle `Roboto-Bold.ttf/Roboto-Regular.ttf` vào target iOS (sửa pbxproj + Info.plist `UIAppFonts`).
> - **Server widgets** (OverlayData.widgets — text/image/stats fractional coords, max 6) — chưa port sang iOS. Android vẽ lớp cuối; nếu admin cấu hình widget thì iOS thiếu.
> - **Frame pacing** — offscreen render chạy trên **main run loop** (CADisplayLink add to `.main`). 24fps bị làm tròn trên 60Hz. Log spam `CMBufferQueue -12764` là rớt frame khi render chậm hơn camera, không gây lỗi. Có thể tối ưu (giảm frameRate khi thermal, đưa overlay bitmap sang GPU compositor riêng) nhưng chưa cần.
> - **"Đang chuyển sang trận mới" giữa lúc live** — có thể là bug hoặc do trận đã finished trên hệ thống. Đã thêm log `[PTLive court]/[PTLive match]` để lần sau xác định.
>
> ### ✅ Cần user làm sau khi pull
> 1. **Deploy backend** (lệnh SSH ở trên) — bắt buộc cho link FB, siết reuse live, ép LIVE_NOW, fix `foundMdatAtom`.
> 2. **Build lại iOS** (Xcode) — chuỗi 10 fix iOS ở trên.
> 3. Test kịch bản: mở app → mất **1 lần** thấy banner "Cannot finalize…" khi vòng upload dọn recording kẹt cũ (fix `607f5a1c` — sau đó sạch). Vào trận **đang chấm điểm** → GO LIVE → mở link trên web/FB. Kỳ vọng: link FB dạng `facebook.com/<page>/videos/…` (không còn `watch/live`), FB chuyển LIVE sau ~10s, overlay + sponsor + tỉ số cập nhật realtime giống Android.
> 4. Nếu tỉ số/tay giao chưa realtime → gửi console log Xcode (filter `PTLive`) — sẽ chỉ ra event nào bị bỏ, decode fail hoặc socket không nhận.
>
> ### 📚 Memories mới phiên này (đọc để hiểu nhanh)
> `[[native-live-ios-rtmp-fb-authtoken]]` `[[native-live-ios-crash-decode-ats]]` `[[native-live-ios-haishinkit-offscreen]]` `[[native-live-ios-score-datapath]]` `[[live-fb-video-unavailable]]`
>
> ---
...
> 10c. **Build native 1.1.16/46 (13/09 sáng)** — Android xong: `~/Desktop/pickletour-android/pickletour-1.1.16.{aab,apk}` (171/125 MB, ký release; icon mới verified in APK). iOS: bump version xong, Xcode workspace mở sẵn cho user Archive & Upload (máy chưa có provisioning profile → không tự Archive CLI). **GOTCHA:** đừng `./gradlew clean` — sẽ vỡ `.cxx` và fail `externalNativeBuildCleanRelease`; thay bằng `rm -rf app/build/{intermediates,outputs} app/.cxx`.
> 10b. **Icon app mới (13/09)** — logo PickleTour badge tròn; `assets/images/icon-v3.png` + `adaptive-icon-v3.png`, app.json icon/adaptiveIcon, 5 appiconset iOS, mipmap Android (foreground 108dp chuẩn). Mobile commit `35ba427`. **Cần build native mới lên store; OTA không đổi icon.** Chi tiết root `../HANDOFF.md`.
