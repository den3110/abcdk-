...
const OPTIMIZED_PUBLIC_DIR = "/uploads/optimized/rankings/kyc";
const OPTIMIZED_DIR = path.join(UPLOADS_ROOT, "optimized", "rankings", "kyc");
const KYC_WIDTH = 1600;
const KYC_HEIGHT = 1600;
const KYC_QUALITY = 82;
...
export async function ensureRankingKycImageUrl(req, raw) {
  ...
  const suffix = crypto
    ...
    .digest("hex")
    .slice(0, 10);
  const outputFileName = `${parsed.name}-${suffix}-${KYC_WIDTH}w.webp`;
  ...
  if (!outputStat || outputStat.mtimeMs < sourceStat.mtimeMs) {
    try {
      ...
      console.error(
        "[rankingKycVariant] optimize failed:",
        error?.message || error
      );
      return fallbackUrl;
    }
