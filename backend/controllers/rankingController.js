        ...
        docs: [
          {
            $addFields: {
              ...
              totalTours: { $ifNull: ["$totalFinishedTours", 0] },
              hasStaffAssessment: { $ifNull: ["$hasStaffAssessment", false] },
              lastFinishedTourAt: "$lastFinishedTourAt",
              lastAssessmentAt: "$lastAssessmentAt",
              lastStaffAssessmentAt: "$lastStaffAssessmentAt",
              colorRank: { $ifNull: ["$colorRank", 3] },
              tierColor: { $ifNull: ["$tierColor", "grey"] },
...
export const getRankingOnlyV2 = asyncHandler(async (req, res) => {
  ...
  /* ===== Khi CÓ từ khoá (và không lọc theo tier): hiện cả tài khoản khớp
     nhưng CHƯA có bản ghi BXH -> 0 điểm, tier xám. Duyệt BXH giữ nguyên. */
  if (
    keywordRaw &&
    userIdsFilter &&
    ...
    try {
      ...
      console.warn("[getRankingOnlyV2] union unranked failed:", e?.message || e);
    }
  }
