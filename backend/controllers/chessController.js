// controllers/chessController.js — Cờ vua
import asyncHandler from "express-async-handler";
import ChessRoom from "../models/chessRoomModel.js";
import {
  startHand,
  applyMove,
  applyResign,
  serializeRoom,
} from "../services/chessEngine.js";
...
async function closeStaleRooms() {
  try {
    ...
    console.error("[chess] closeStaleRooms:", err?.message || err);
  }
}
...
function emitRoomTo(room, event, payload) {
  try {
    getIO?.()?.to(`chess:room:${room._id}`).emit(event, payload);
  } catch {}
}
...
export const listChessRooms = asyncHandler(async (_req, res) => {
  await closeStaleRooms();
  const rooms = await ChessRoom.find({ status: "open" })
...
export const createChessRoom = asyncHandler(async (req, res) => {
  const b = req.body || {};
  const name = String(
...
export const getChessRoom = asyncHandler(async (req, res) => {
  const room = await ChessRoom.findById(req.params.id).populate(
    "seats.user",
    USER_FIELDS,
  );
  if (!room) {
...
export const sitChessRoom = asyncHandler(async (req, res) => {
  const room = await ChessRoom.findById(req.params.id);
  if (!room) {
    res.status(404);
    throw new Error("Không tìm thấy bàn");
  }
...
export const leaveChessRoom = asyncHandler(async (req, res) => {
  const room = await ChessRoom.findById(req.params.id);
  if (!room) {
    res.status(404);
    throw new Error("Không tìm thấy bàn");
...
export const startChessHand = asyncHandler(async (req, res) => {
  const room = await ChessRoom.findById(req.params.id);
  if (!room) {
    res.status(404);
    throw new Error("Không tìm thấy bàn");
...
// POST /api/chess/rooms/:id/move  { from, to, promotion? }
export const chessMove = asyncHandler(async (req, res) => {
  const room = await ChessRoom.findById(req.params.id);
  if (!room) {
    res.status(404);
    throw new Error("Không tìm thấy bàn");
  ...
  try {
    applyMove(room, seat.seatIndex, { from, to, promotion });
  } catch (err) {
    res.status(400);
    throw err;
  }
...
// POST /api/chess/rooms/:id/resign
export const chessResign = asyncHandler(async (req, res) => {
  const room = await ChessRoom.findById(req.params.id);
  if (!room) {
    res.status(404);
    throw new Error("Không tìm thấy bàn");
...
export const chatChessRoom = asyncHandler(async (req, res) => {
  ...
  room.lastActivityAt = new Date();
  await room.save();
  emitRoomTo(room, "chess:room:chat", { roomId: String(room._id), message: msg });
  res.json({ ok: true, message: msg });
});

const inviteLog = new Map();
export const inviteChessRoom = asyncHandler(async (req, res) => {
  ...
  const room = await ChessRoom.findById(req.params.id).select("_id name");
  if (!room) {
    res.status(404);
    throw new Error("Không tìm thấy bàn");
  }
  const title = "♟️ Mời chơi Cờ Vua";
  const body = `${req.user.nickname || req.user.name} mời bạn vào bàn "${room.name}"`;
  const data = { url: `/chess/${room._id}`, roomId: String(room._id) };
  try {
