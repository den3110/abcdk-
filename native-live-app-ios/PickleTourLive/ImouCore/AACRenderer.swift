...
public final class AACRenderer {
  ...
  /// LIVE: nếu set, PCM giải mã đẩy vào callback (→ HaishinKit) THAY VÌ phát ra loa.
  ...
  private func drainDecodeQueue() {
    ...
    while true {
      ...
      if let cb = onPCMBuffer { cb(pcm) }        // LIVE: đẩy vào HaishinKit
      else { player.scheduleBuffer(pcm, completionHandler: nil) }  // hiển thị/PiP: phát loa
      dbgDecoded += 1
      if dbgDecoded % 100 == 0 {
